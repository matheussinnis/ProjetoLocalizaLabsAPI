...
namespace UnitTests.Web.Controllers
{
    ...
    {
        private QuotationController _controller { get; set; }
        private Mock<IQuotationService> _service { get; set; }
        ...
        public async void TestGetAll()
        {
            _service.Setup(s => s.GetAllAsync()).Returns(
                Task.FromResult(Enumerable.Empty<Quotation>())
            );

            var response = await _controller.GetAll();

            Assert.Equal(200, response.StatusCode);
            Assert.Equal(Enumerable.Empty<Quotation>(), response.Value);
        }
        ...
        public async void TestFind()
        {
            ...
            var modelMock = new Quotation() { Id = guid };
